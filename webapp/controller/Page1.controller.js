sap.ui.define(["sap/ui/core/mvc/Controller",
	...
	//Initialize the variable to store the image reference from URL parameter
	var imgUrlFromURL; 
	...
	return BaseController.extend("com.sap.build.standard.prototipo1.controller.Page1", {
						...
						if (Object.keys(oParam).length !== 0) {
							for (var prop in oParam) {
								if (prop !== "sourcePrototype") {
									//Store the Image link in the variable
									if (prop == "ProdImgURL") { 
		...
		webBridgeSuccessMsg: function () {
			try {
				sap.sbo.webbridge.showSuccess("This is a success message"); //thiago.mashup.BusinessOne.B1
			} catch (err) {
		...
		webBridgeErrorMsg: function () {
			try {
				sap.sbo.webbridge.showError("This is an error message"); //thiago.mashup.BusinessOne.B1
			} catch (err) {
				MessageToast.show("Error: " + err.message);
			}
		},
		...
		webBridgeNoteMsg: function () {
			try {
				sap.sbo.webbridge.showNote("This is a note"); //thiago.mashup.BusinessOne.B1
			} catch (err) {
				MessageToast.show("Error: " + err.message);
			}
		},
		...
		similarFromURL: function (oEvent) {
			...
			var processResult = function (data) {
				// merge with existing results -  working with B1 only in this case
				var result = oView.getModel("demo").getProperty("/result");
				if (result) {
					result.push.apply(result, data.b1);
				} else {
			...
			//Gets image link (URL) from URL parameter
			var oB1ImgURL = oEvent.getSource().mProperties.text;
			if (oB1ImgURL) {
				imgUrlFromURL = oB1ImgURL;
			}
			
			//Formats body for the ajax call
			var body = {
				"url": imgUrlFromURL
			};
			
			//If image link is sent as URL parameter, 
			//then call the nodejs on Cloud Foundry to get similar items
			if (imgUrlFromURL) {
				$.ajax({
		...
		//Pending: Implement GeoSpatial based on store address
